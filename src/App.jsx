...
// Canvas lógico — la resolución interna no cambia
...
const DRIVES = [
  ...
  { id: "mie", label: "FEAR",           color: "#ff4d6d", uc: false, ba: 200, bm: 0.38, af: 0.28, aa: 40, mf: 0.35, ma: 0.22, ph: 1.2 },
  { id: "des", label: "DESIRE",         color: "#f5a040", uc: false, ba: 75,  bm: 0.44, af: 0.14, aa: 24, mf: 0.22, ma: 0.18, ph: 0.7 },
  { id: "soc", label: "SOCIAL IMPULSE", color: "#00e676", uc: false, ba: 308, bm: 0.28, af: 0.52, aa: 50, mf: 0.42, ma: 0.20, ph: 2.1 },
  { id: "hab", label: "HABIT",          color: "#b44fff", uc: false, ba: 150, bm: 0.36, af: 0.07, aa: 18, mf: 0.09, ma: 0.14, ph: 3.5 },
];
...
function cohLabel(c) {
  if (c>=0.88) return "full"; if (c>=0.70) return "high";
  if (c>=0.50) return "medium"; if (c>=0.30) return "low"; return "opposite";
}

function interpText(fp, ali, coup, noise, coh) {
  if (noise < 0.12) return "Internal noise at minimum: the unconscious vectors are static. You are seeing the base configuration of this consciousness — the default position before any internal agitation.";
  if (coh < 0.25)   return "Intention points in the opposite direction from the resultant. Unconscious impulses dominate the actual trajectory. There is a great deal of energy — but moving contrary to what is consciously wanted.";
  if (ali < 0.28)   return "Internal vectors scatter in opposing directions. The resultant oscillates without bearing — not from lack of energy, but from incoherence. The field cannot amplify what it cannot find form in.";
  if (ali >= 0.65 && fp >= 0.63 && coup >= 0.60) return "Maximum convergence: aligned vectors, active field, and deep coupling. The resultant is long, stable, and coherent. This is the geometry of effective action in the MVC.";
  if (coh >= 0.85 && ali >= 0.55) return "High I→R coherence: conscious intention and resultant point together. The unconscious vectors amplify the chosen direction.";
  if (ali >= 0.55 && fp < 0.40)   return "High internal coherence, but the field attenuates the projection. Moving toward a cyan zone would multiply an already well-formed vector.";
  if (coup < 0.15)  return "Minimum coupling: this consciousness has not yet built history with this field. Even if the resultant is coherent, its projection into the world is still provisional.";
  return "The field evolves slowly — zones of potency and attenuation shift. Staying still is not staying in the same field.";
}
...
export default function MVCVectorInterno() {
  ...
  useEffect(() => {
    ...
    function tick(ts) {
      ...
      if (isPaused) { ctx.save(); ctx.font=`bold 11px ${T.mono}`; ctx.fillStyle="rgba(0,220,190,0.45)"; ctx.textAlign="right"; ctx.fillText("— PAUSED —",CW-10,CH-10); ctx.restore(); }
      ctx.save(); ctx.font=`10px ${T.mono}`; ctx.fillStyle="rgba(0,200,170,0.26)";
      ctx.textAlign="center"; ctx.fillText("↑ transcendence",CW/2,15);
      ctx.fillText("↓ matter",CW/2,CH-7);
      ctx.textAlign="left";  ctx.fillText("← origin",6,CH/2+4);
      ctx.textAlign="right"; ctx.fillText("other →",CW-6,CH/2+4);
      ctx.restore();
      frameRef.current++;
      if (frameRef.current%6===0) {
        const eff=res.normMag*fp*S.coup;
        setMetrics({
          zona:   fp>=0.63?"HIGH POTENCY":fp>=0.44?"NEUTRAL ZONE":"ATTENUATED ZONE",
  ...
  return (
    <div style={{ background:T.bg, padding:"0.75rem", display:"flex", flexDirection:"column", gap:8,
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0 }}>
        <div>
          ...
          <p style={{ fontFamily:T.mono, fontSize:9, letterSpacing:"0.07em", fontStyle:"italic", color:T.textDim, margin:"3px 0 0" }}>
            does the field modulate consciousness — or does consciousness construct the field?
          </p>
        </div>
      ...
      {/* ── Cuerpo principal: canvas + controles ── */}
      <div style={{ display:"flex", gap:10, flex:1, minHeight:0 }}>

        {/* Canvas — ocupa todo el alto disponible, proporción 1:1 */}
        ...
        {/* Columna de controles — 2×2 grid + interpretación */}
        <div style={{ flex:1, minWidth:0, display:"flex", flexDirection:"column", gap:8, overflow:"hidden" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, flex:1, minHeight:0 }}>
            ...
            {/* Panel 1 — métricas */}
            <Panel title="Force · Coupling · Reach">
              <MetricRow label="ZONE"            value={metrics.zona} />
              <MetricRow label="FIELD POTENCY"   value={metrics.fp} />
              <MetricRow label="ALIGNMENT"       value={metrics.ali} />
              <MetricRow label="I→R COHERENCE"   value={metrics.coh} valueColor={cohColor} />
              <MetricRow label="COUPLING"        value={metrics.coup} />
              <MetricRow label="EFFECTIVE REACH" value={metrics.eff} highlight />
            </Panel>

            {/* Panel 2 — intención consciente */}
            <Panel title="Conscious Intention">
              <SliderRow label="DIRECTION" min={0}   max={359} value={angle} displayValue={`${angle}°`} onChange={setAngle} />
              <SliderRow label="WILL"      min={5}   max={100} value={vol}   displayValue={`${vol}%`}   onChange={setVol} />
              <SliderRow label="INT NOISE" min={0}   max={100} value={noise} displayValue={`${noise}%`} onChange={setNoise} />
            ...
            {/* Panel 3 — campo */}
            <Panel title="Existential Potential Field">
              <SliderRow
                label="FIELD SPEED" min={0} max={100} value={fieldSpeed}
                displayValue={fieldSpeed<15?"slow":fieldSpeed<50?"normal":fieldSpeed<80?"fast":"maximum"}
              ...
              <p style={{ fontFamily:T.mono, fontSize:9, letterSpacing:"0.05em", color:T.textDim, margin:"6px 0 0", lineHeight:1.6 }}>
                the field evolves independently<br/>of the agents inhabiting it
              </p>
              {/* Leyenda de colores */}
              <div style={{ marginTop:8, paddingTop:6, borderTop:`0.5px solid ${T.borderDim}` }}>
                {[["cyan","#00d8c8","zone of potency"],["violet","#9060cc","zone of attenuation"]].map(([,c,l])=>(
            ...
            <Panel title="Unconscious Vectors">
              {unconscious.map(d=>(
              ...
              <div style={{ marginTop:7, paddingTop:5, borderTop:`0.5px solid ${T.borderDim}`, display:"flex", alignItems:"center", gap:7 }}>
                ...
                <span style={{ fontFamily:T.mono, fontSize:9, letterSpacing:"0.08em", color:T.textSec, flex:1 }}>RESULTANT</span>
                <div style={{ flex:"0 0 36px", height:2, background:"rgba(0,200,175,0.10)", borderRadius:1, overflow:"hidden" }}>
              ...
              <div style={{ marginTop:8, paddingTop:6, borderTop:`0.5px solid ${T.borderDim}`,
                ...
                colored = unconscious<br/>
                cyan = resultant<br/>
                drag node → change zone
              </div>
            </Panel>
          </div>

          {/* Interpretación — franja inferior */}
